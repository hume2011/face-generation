...
# 该项目使用生成式对抗网络（Generative Adversarial Nets）来生成新的人脸图像。
# ### 获取数据
# 该项目将使用以下数据集：
# - MNIST
# - CelebA
# 
# 第一次使用 GANs,先在 MNIST 数据集上测试你的 GANs 模型，以更快的评估所建立模型的性能。
...
# ### MNIST
# [MNIST](http://yann.lecun.com/exdb/mnist/) 是一个手写数字的图像数据集。可以更改 `show_n_images` 探索此数据集。
...
# ### CelebA
# [CelebFaces Attributes Dataset (CelebA)](http://mmlab.ie.cuhk.edu.hk/projects/CelebA.html) 是一个包含 20 多万张名人图片及相关图片说明的数据集,用此数据集生成人脸.。
...
# 检查是否使用正确的 TensorFlow 版本，并获取 GPU 型号
...
# ### 输出显示
# 使用该函数可以显示生成器 (Generator) 在训练过程中的当前输出，这会帮评估 GANs 模型的训练程度。
...
# ### 训练
# 部署 `train` 函数以建立并训练 GANs 模型。使用以下你已完成的函数：
...
# ### MNIST
# 在 MNIST 上测试 GANs 模型。经过 2 次迭代，GANs 能够生成类似手写数字的图像。
